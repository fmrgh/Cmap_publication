...
le_list <- final_dfs[-grep("\\.High$", names(final_dfs))]
#le_list[[19]]<- final_dfs[["Androstendion.High"]]
le_list<- le_list[-c(14,3,4,29,33,35,37,39)]

################################
#####select for each top 20 DEGs#####
# Function to select top 20 rows by pvalue
get_top_20 <- function(df) {
  df[order(df$padj), ][1:100, ]
}
get_top_log2fold <- function(df) {
  # Order by absolute values of log2FoldChange and get the top 2
  top_genes <- df[order(abs(df$log2FoldChange), decreasing = TRUE), ][1:100, ]
  
  return(top_genes)
}
get_top_stat <- function(df, n = 100) {
...
###################################
top_20_list <- lapply(le_list, get_top_20)
top_20_list<- lapply(top_20_list, na.omit)
top_20_log2fold <- lapply(le_list, get_top_log2fold)
top_20_log2fold<- lapply(top_20_log2fold, na.omit)
top_20_stat <- lapply(le_list, get_top_stat)
...
rownames_vector_all <- unique(unlist(lapply(le_list, rownames)))



#######################################
filtered_data_list <- lapply(le_list, function(df) df[rownames(df) %in% rownames_vector_all, ])
modified_data_list <- list()
names(filtered_data_list) <- sub("\\..+$", ".High", names(filtered_data_list))
...
rownames(conn_mat) <- sub("\\..*$", "", rownames(conn_mat))
